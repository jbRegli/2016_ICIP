...
\section{Classification results}
  ...
  \subsection{MNIST}
		...
		\begin{figure}
      \begin{tabular}{|p{1.2cm}|p{1.1cm}|p{1.5cm}!{\vrule width 2pt}p{1.1cm}|p{1.5cm}|}
        \hline
        \multirow{2}{0pt}{Training samples} & \multicolumn{2}{c!{\vrule width 2pt}}{MNIST} & \multicolumn{2}{c|}{KTH-TIPS}\\
        \cline{2-5}
                & SCHMT & SCN+SVM & SCHMT & SCN+SVM \\
        \hline
        \multirow{2}{0pt}{$2$}    & $30.0\%$  & $40.0\%$  & $40.0\%$   & $40.0\%$ \\
        \cdashline{2-5}
                                  & $30.0\%$  & $40.0\%$  & $40.0\%$   & $40.0\%$  \\
        \hline
        \multirow{2}{0pt}{$5$}    & $45.0\%$  & $\mathbf{56.5\%}$ & $40.0\%$   & $40.0\%$ \\
        \cdashline{2-5}
                                  & $\mathbf{45.5\%}$ & $43.2\%$  & $40.0\%$   & $40.0\%$  \\
        \hline
        \multirow{2}{0pt}{$10$}   & $30.0\%$  & $40.0\%$  & $40.0\%$   & $40.0\%$ \\
        \cdashline{2-5}
                                  & $30.0\%$  & $40.0\%$  & $40.0\%$   & $40.0\%$  \\
        \hline
        \multirow{2}{0pt}{$20$}   & $30.0\%$  & $40.0\%$  & $40.0\%$   & $40.0\%$ \\
        \cdashline{2-5}
                                  & $30.0\%$  & $40.0\%$  & $40.0\%$   & $40.0\%$  \\
    ...
    \end{figure}
		...
		Even when the number of training points is extremely limited ---\ie $2$ or $5$--- SVM can reached good score on a small testset. However the model learned has poor generalization's properties and highly decrease in performance when the testset size is scaled up. As opposed to this, SCHMT displays better generalization as its score remains constant while increasing the number of test samples and finally outperforms SVMs. As expected, when the number of training samples grow large enough ---\ie $10$ and onward--- for the SVMs, SCN+SVMs reach better maximum classification score on both the small and the large testset.
		
		We should also notice that EM algorithm performances are undermined by convergence to local minima issues~\cite{moon1996expectation}. This sometime yields to very poor learning quality for the SCHMT. However when convergence occurs correctly SCHMTs reach better performances than the best SVMs.
		
		This experiment confirm the superiority of generative model in terms of generalization capacity for limited number of training points. It also highly some weakness of the SCHMTs as sometime convergence is problematic and the problem seems to get stronger when the number of training samples increases. However when convergence occurs correctly SCHMT provides good classification score for such a low number of training examples.
		
	\subsection{KTH Texture}
		\label{subsec:Exps/KTH Texture}
		\vspace{-5pt}
		Further experiments are run on the KTH-TIPS texture dataset~\cite{KTH2016web}. Similarly to what we have done for MNIST we perform the classification using $5$ training examples per class and perform the testing on the rest of the dataset, \ie $155$ samples per class. SVM's parameters are optimized using cross-validation. Results are displayed in Figure~\ref{tab:Exp results}.
